    ...
    def maps_data(self):
        ...
        if (HOSP_SCENARIO==True):
            ...
            with open(join(MAP_DATA_DIR, PRIORITIES_POINTS), "rb") as f:
                self._all_priorities_points = pickle.load(f)
            with open(join(MAP_DATA_DIR, PRIORITIES_CELLS), "rb") as f:
                self._all_priorities_cells = pickle.load(f)
...
class Saver(object):
    ...
    def maps_data(self, obs_points, points_matrix, cells_matrix, obs_cells, cs_cells, enb_cells, cs_points, eNB_point, hosp_points=None, hosp_cells=None, all_priorities_points=None, all_priorities_cells=None):
        # Data map made by Points and Cells:
        ...
        if ( (hosp_cells!=None) and (all_priorities_points!=None) and (all_priorities_cells!=None) ):
            with open(join(MAP_DATA_DIR, HOSP_CELLS), 'wb') as f:
                pickle.dump(hosp_cells, f)
            with open(join(MAP_DATA_DIR, PRIORITIES_POINTS), 'wb') as f:
                pickle.dump(all_priorities_points, f)
            with open(join(MAP_DATA_DIR, PRIORITIES_CELLS), 'wb') as f:
                pickle.dump(all_priorities_cells, f)
