        ...
        for x_agent in np.arange(0, map_width+1, 1): #range(map_width)
            ...
            for y_agent in np.arange(0, map_length+1, 1): #range(map_length)
                ...
                if (ANALYZED_CASE == 1):
                    ...
                    elif (HOSP_SCENARIO == True):
                        for prior in HOSP_PRIORITIES:
                            ...
                            current_uav_q_table[((x_agent, y_agent), 1, prior)] = [0 for action in range(n_actions)]   #da vedere se mettere 0
                            current_uav_q_table[((x_agent, y_agent), 0, prior)] = [0 for action in range(n_actions)]  # da vedere se mettere 0
...
if (HOSP_SCENARIO==True):
    ...
    tg = [RestrainingBolt.make_uavs_goal() for uav in range(N_UAVS)] #tg = RestrainingBolt.make_uavs_goal()
...
# -------------------------PROVA2----------------------------------------------------------------------------
    ...
    '''if ((best_policy[0]/ 20) >= 0.95):
        ...
        if EPSILON > EPSILON_MIN:
            ...
            EPSILON_MIN'''
    if (REDUCE_ITERATION_PER_EPISODE == True):
        if (done and uavs_episode_reward[UAV]/20 >= 0.95): #best_policy[UAV] ):
            ITERATIONS_PER_EPISODE = current_iteration
        if(ITERATIONS_PER_EPISODE < 90):
            print("ITERATIONS_PER_EPISODE:", ITERATIONS_PER_EPISODE)
    ...
    if (NUMBER_ITER < 90):
        print("Number of iterations (best policy):", NUMBER_ITER, "Episode: ", EPISODES_BP)
...
for i in range(len(traj_j_ID)):
    ...
    traj_j_ID[i].insert(4, UAV_HEIGHT_Z)
...
try:
    ...
    print("Error: %s : %s" % (dir_path, e.strerror))
dir_path = './envs/custom_env_dir/__pycache__'
try:
    shutil.rmtree(dir_path)
except OSError as e:
    print("Error: %s : %s" % (dir_path, e.strerror))
