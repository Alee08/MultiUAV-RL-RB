...
#hosp_pos = [(6, 2), (4, 6), (7,5), (2,4), (5, 9), (3, 2), (8,5), (3,4), (7, 7), (5, 7)] #, (8,6), (3,5), (9, 3), (7, 7), (0,0), (0,1), (5, 2), (3, 6), (6,5), (0,4), (0, 2), (5, 0), (1,5), (1,4), (1, 3), (5, 1), (2,6), (1,5), (1, 3), (7,0), (0,2), (1,1)]
    ...
    if randomm == True:
        ...
        def hosp_priority(hospitals, matrix_elems, points=True):
            ...
            for hosp in hospitals:
                ...
                if (points==True):
                    ...
                    if N_HOSP == 2:
                        ...
                        if count%2 != 0:
                            ...
                            # The priority for the sender hospital is the same set for the receiver hospital communicating with the current one
                            priority_idx += 1
                            receiver = False
                ...
                count += 1
                #print(get_color_name(current_priority))
        ...
        def hosp_priority(hospitals, matrix_elems, points=True):
            ...
            for hosp in hospitals:
                ...
                if (points == True):
                    ...
                    current_priority = hosp._priority
                    #print("hosp._x_coordhosp._x_coordhosp._x_coord", hosp._x_coord)
                    priority_idx += 1
...
class Cell:
    ...
    def get_bip_counts(self):
        """Get counts."""
        return self.counts[y_coord][x_coord]

    def do_beep(self ):
        """Do a beep for a cell."""
        self.counts[y_coord][x_coord] += 1
            ...
            for ob in obs_points:
                if (hosp_assigned==n_hosp):
                    ...
                    if randomm ==True:
                        ...
                        #hosp_pos = [(3, 2), (9, 9), (5, 5), (6, 6)]
                        ob._x_coord = hosp_pos[i][0]
                        ob._y_coord = hosp_pos[i][1]
                        points_matrix[ob._y_coord][ob._x_coord] = ob
                        points_matrix[ob._y_coord][ob._x_coord]._status = self._hosp_in  # --> SIDE-EFFECT on 'points_matrix' --> 'points_matrix' will contain obstacles and hospitals (that are obviously obstacles as well).
                        hosp_points[hosp_assigned] = ob
                        obs_points.remove(ob)
    ...
    def compute_cell_matrix(self, points_matrix, actual_z_enb, area_width, area_height, cell_res_row, cell_res_col):
        ...
        for i in range(self._cells_rows):
            for j in range(self._cells_cols):
                ...
                receiver_points = [point.receiver for point in current_points]
                # You should avoid generating receiver and sender hospitals at a distance lower than the chosen resolution (if you choose a grid-resolution greater than the minimum one): 
                if True in receiver_points:
                    receiver = True
                elif False in receiver_points:
                    receiver = False
                else:
                    receiver = None
                    ...
                    if (UNLIMITED_BATTERY == True):
                        if (HOSP_SCENARIO==False):
                            ...
                            if (self._hosp_in in status_points):
                                ...
                                cells_matrix[i][j] = Cell(self._free, current_points, j, i, 0, [], priority_type)
                            cells_matrix[i][j].receiver = receiver
                    else:
...
if __name__ == '__main__':
    ...
    cells_matrix = env.compute_cell_matrix(points_matrix, 0, env._area_width, env._area_height, env._cell_res_row, env._cell_res_col)
    ...
    if HOSP_SCENARIO == True:
        CS_points = []
        obs_points = []
    #print(eNB_point, CS_points, obs_points, obs_cells, points_matrix, cells_matrix)
    ...
    # Saving:
    saver = Saver()
    saver.maps_data(obs_points, points_matrix, cells_matrix, obs_cells, cs_cells, eNB_cells, CS_points, eNB_point, hosp_points, hosp_cells, all_priorities_points, all_priorities_cells)
    '''saver.maps_data(obs_points, points_matrix, cells_matrix, obs_cells, cs_cells, eNB_cells,
                    hosp_points, hosp_cells, all_priorities_points, all_priorities_cells)'''
    if HOSP_SCENARIO == True:
        users_xyz = []
        initial_centroids = []
        initial_clusters_radiuses = []
        initial_clusterer = []
